/**
 * ENS SDK - Browser Compatible Version
 * 
 * This file provides a browser-compatible version of the ENS SDK
 * that handles the window/localStorage checks properly
 */
...
// Types (re-exported from main SDK)
...
const NETWORKS: Record<string, NetworkConfig> = {
  mainnet: {
    rpc: process.env.NEXT_PUBLIC_ETHEREUM_RPC_URL || process.env.ETHEREUM_RPC_URL || 'https://cloudflare-eth.com',
    registry: '0x00000000000c2e074ec69a0dfb2997ba6c7d2e1e',
    controller: '0x253553366da8546fc250f225fe3d25d0c782303b',
    name: 'Ethereum Mainnet'
  },
  holesky: {
    rpc: process.env.NEXT_PUBLIC_HOLESKY_RPC_URL || process.env.HOLESKY_RPC_URL || 'https://ethereum-holesky.publicnode.com',
    registry: '0x00000000000c2e074ec69a0dfb2997ba6c7d2e1e',
    controller: '0xfce6ce4373cb6e7e470eaa55329638acd9dbd202',
    name: 'Holesky Testnet'
  },
  sepolia: {
    rpc: process.env.NEXT_PUBLIC_SEPOLIA_RPC_URL || process.env.SEPOLIA_RPC_URL || 'https://ethereum-sepolia.publicnode.com',
    registry: '0x00000000000c2e074ec69a0dfb2997ba6c7d2e1e',
...
const RESOLVER_ABI = [
  ...
  { inputs: [{ name: 'node', type: 'bytes32' }], name: 'contenthash', outputs: [{ type: 'bytes' }], stateMutability: 'view', type: 'function' }
];
...
export class ENSSDK {
  ...
  private async initNetwork(network: string): Promise<NetworkConfig> {
    ...
    if (!config.rpc) {
      throw new Error(`Missing RPC URL for ${network}. Set NEXT_PUBLIC_${network.toUpperCase()}_RPC_URL in .env.local`);
    }
  ...
  /**
   * Load internal names from localStorage (browser-safe)
   */
  private loadInternalNames(): void {
    if (typeof window !== 'undefined' && typeof window.localStorage !== 'undefined') {
      try {
        const stored = window.localStorage.getItem('ens-internal-names');
        if (stored) {
  ...
  /**
   * Save internal names to localStorage (browser-safe)
   */
  private saveInternalNames(): void {
    if (typeof window !== 'undefined' && typeof window.localStorage !== 'undefined') {
      try {
        const names = Array.from(this.internalNames.values());
        window.localStorage.setItem('ens-internal-names', JSON.stringify(names));
      } catch (e) {
  ...
  /**
   * Fetch all ENS records for a name
   */
  private async fetchENSRecords(node: string, resolver: ethers.Contract): Promise<ENSRecords> {
    const records: ENSRecords = {};
    
    // Standard text record keys to check
    const textKeys = [
      'description', 'url', 'avatar', 'notice', 'keywords',
  ...
  /**
   * Resolve a name to address (ENS or internal) with full records
   */
  public async resolve(name: string, network: string = 'mainnet'): Promise<ENSResult | null> {
    ...
    try {
      ...
      // Fetch all ENS records
      const records = await this.fetchENSRecords(node, resolver);
  ...
  /**
   * Reverse resolve an address to name (ENS or internal) with full records
   */
  public async reverse(address: string, network: string = 'mainnet'): Promise<ENSResult | null> {
    ...
    try {
      ...
      // Fetch all ENS records for the resolved name
      const nameNode = this.namehash(name.toLowerCase());
      const records = await this.fetchENSRecords(nameNode, resolver);
...
// Enhanced methods for fetching specific records
export const getTextRecord = async (name: string, key: string, network?: string) => {
  try {
    const result = await ensSDK.resolve(name, network);
    return result?.records?.[key] || null;
  } catch (e) {
    return null;
  }
};

export const getAllRecords = async (name: string, network?: string) => {
  const result = await ensSDK.resolve(name, network);
